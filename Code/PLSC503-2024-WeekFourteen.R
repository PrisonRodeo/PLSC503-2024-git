...
P<-c("readr","MASS","psych","ggplot2","mfx","margins",
     "aod","car","modelsummary","marginaleffects")
...
NewDahl<-read_csv("https://raw.githubusercontent.com/PrisonRodeo/PLSC503-2024-git/master/Data/Dahl2021.csv")

NewDahl<-NewDahl[complete.cases(NewDahl),]
row.names(NewDahl)<-NewDahl$Year
describe(NewDahl)

pdf("NewDahlScatterplotMatrix2.pdf",10,8)
...
pdf("AltOutOfSampleHats.pdf",6,5)
...
PC<-glm(YPois~X,family="poisson") # Poisson (correct)
NBE<-glm.nb(YPois~X)              # NB (extraneous)
PW<-glm(YNB~X,family="poisson")   # Poisson (wrong)
NBC<-glm.nb(YNB~X)                # NB (correct)

# Plot:

pdf("OverdispersionPlot.pdf",6,4)
p<-modelplot(list("Poisson (Correct)"=PC,
                  "NB (extraneous)"=NBE,
                  "Poisson (Incorrect)"=PW,
                  "NB (Correct)"=NBC)) + 
  geom_vline(xintercept=0,linetype="dashed") +
  theme_classic()
p
